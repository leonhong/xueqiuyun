	            ...
	            <div class="" >
	                ...
	                <div class="col1">
	                    <div class="btn-group btnnav" data-toggle="buttons">
	                        <label class="btn zu-top-nav-link active">
	                            <input type="radio" name="options" id="hs"> 沪深
	                        </label>
	                        <label class="btn zu-top-nav-link">
...
$db_conf['prot'] = '3306';
                ...
                if(!empty($_GET['key'])){
                    $where .= " and (a.`code` like '%{$_GET['key']}%' or b.stock_name like '%{$_GET['key']}%') ";
                }
                ...
                if(!empty($name) && !empty($sort)){
                    switch($name){
                        case 'time':
                            $order = " order by b.stock_time $sort";
                            break;
                        case 'gujia':
                            $order = " order by b.stock_price $sort";
                            break;
                        case 'rank_all':
                ...
                //获取数据总数
                $sql = "select a.*,b.* from xl_hs_huizong as a left join xl_sh_real_time_share_price as b on a.`code`=b.`stock_code` $where $order";
                ...
                //$sql = "select a.*,b.* from xl_hs_huizong as a left join xl_sh_real_time_share_price as b on a.`code`=b.`stock_code` $where $order  limit $begin,$prepage";
                $sql = "select a.*,b.*,c.* from xl_hs_huizong as a left join xl_sh_real_time_share_price as b on a.`code`=b.`stock_code` left join xl_hs_value as c on a.`code`=c.`code`  $where $order  limit $begin,$prepage";
                /*echo $sql;
                exit;*/
                $list = $db->select($sql);
                ...
                ?>
                    ...
                    ?>
		                    <tr <?php if($v['is_hk']=='1'){ ?>style="background-color: #e8f3fd;" item="1" <?php } ?>>
		                        <td><a href="hs_detail.php?code=<?php echo $v['code']; ?>" class="active" data-toggle="class"><?php echo $v['code'];?></a></td>
		                        <td><a href="hs_detail.php?code=<?php echo $v['code']; ?>" class="active" data-toggle="class">
		                        	<?php 
		                        		if(preg_match("/^[a-zA-Z\s]+$/",$v['name'])){
		                        			echo substr($v['stock_name'],0,8);
		                        		}
                                        elseif(strpos($v['stock_name'],'(')){
                                            echo substr($v['stock_name'],0,strripos($v['stock_name'],'('));
                                        }
                                        else{
		                        			echo substr($v['stock_name'],0,12);
		                        		}
        ...
        <div class="table-responsive"id="list">
            <table class="table table-striped b-light text-sm" >
                ...
                foreach($list as $k=>$v) {
                    ...
                    <tr <?php if($v['is_hk']=='1'){ ?>style="background-color: #e8f3fd;" item="1" <?php } ?>>
                        <!--<td><a href="hs_detail.php?code=<?php /*echo $v['code']; */?>" class="active" data-toggle="class"><?php /*echo $v['code'];*/?></a></td>
                        <td><?php /*echo $v['stock_name'];*/?></td>-->
                        <td><?php echo empty($v['stock_time'])?'<font color="#fa554">停牌</font>':$v['stock_time'];?></td>
                        <td><?php echo empty($v['stock_price'])?'<font color="#fa554">0.00</font>':$v['stock_price'];?></td>
            ...
            <div class="row">
                ...
                <div class="text-right text-center-xs" >
                    <ul class="pagination pagination-sm m-t-none m-b-none">
                        <li><a href="/hs_table.php?page=<?php echo ($page-1)>0?($page-1):1; ?>&key=<?php echo $_GET['key'];?>&type=<?php echo $_GET['type'];?>&htype=<?php echo $_GET['htype'];?>&name=<?php echo $_GET['name'];?>&sort=<?php echo $_GET['sort'];?>"><i class="fa fa-chevron-left"></i></a></li>
                        <?php for($i=1;$i<=$pagecount;$i++){ ?>
                            <li><a href="/hs_table.php?page=<?php echo $i;?>&key=<?php echo  $_GET['key'];?>&type=<?php echo $_GET['type'];?>&htype=<?php echo $_GET['htype'];?>&name=<?php echo $_GET['name'];?>&sort=<?php echo $_GET['sort'];?>" <?php if($i==$page){ ?> style="background-color: #eeeeee;color: #188fff;"<?php } ?> ><?php echo $i; ?></a></li>
                        <?php } ?>
                        <li><a href="/hs_table.php?page=<?php echo ($page+1)<=$pagecount?($page+1):$pagecount; ?>&key=<?php echo $_GET['key'];?>&type=<?php echo $_GET['type'];?>&htype=<?php echo $_GET['htype'];?>&name=<?php echo $_GET['name'];?>&sort=<?php echo $_GET['sort'];?>"><i class="fa fa-chevron-right"></i></a></li>
                    </ul>
                </div>
            </div>
        </footer>
...
//		$('.select2-search').hide();
//		$("#formula").on("select2-open", function (e) {//设置select2下拉位置 
//		 	if($(window).width()>510){
//		 		$("#select2-drop").css({left:$(".col1").width()+$("#s2id_formula").position().left+36})
//		 	}
//		 });
		var scrollleft=200;
        $(window).scroll(function() {
		    if(($("#list").offset().top-$("body").scrollTop())<0){
				...
				$(".list_left thead th:eq(0) div").css({"width":"56px"})
		    	$(".list_left thead th:eq(1) div").css({"width":"78px"})
		    	set_wh()
		    	if($(".append").length==0){
		 ...
		 $(document).bind('keyup', function(event) {//搜索回车事件
