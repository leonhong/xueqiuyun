...
$code = $_GET['code'];
//获取股票详情
$sql = "select * from xl_hs_detail where code='$code'";
$codedetail = $db->selectOne($sql);
if(!$codedetail){
        header("location:/nofound.html");
}
$sql = "select * from xl_sh_real_time_share_price where stock_code='$code'";
$codedetail = $db->selectOne($sql);
$noewyear = date('Y');
//获取年份
$sql = "select distinct `year` from xl_hs_detail where code='$code' and `year`!=$noewyear and roe !='0' order by `year`  desc";
...
<section class="vbox">
    <section class="panel panel-default">
        ...
        <div class="wrapper hktitle">
            <h3><?php echo $codedetail['stock_name']; ?>(<?php echo $code;?>)</h3>
            <h1><?php echo empty($codedetail['stock_price'])?'<font color="#fa554">停牌</font>':$codedetail['stock_price'];?></h1>
            <small><?php echo empty($codedetail['stock_time'])?'<font color="#fa554">停牌</font>':$codedetail['stock_time'];?></small>
        </div>
        <?php
        $sql = "select * from xl_hs_huizong where code='$code'";
...
<script type="text/javascript">
    ...
    function getoption(title,key)
    {
        ...
        var shuju = [<?php $i=0; foreach(array_reverse($yearlist) AS $key=>$y){$year = $y['year'];$sql = "select * from xl_hs_detail where code='$code' and year='$year'";$row = $db->selectOne($sql);  echo $i=='0'?sprintf('%.3f',($row[$k]*100)):','.sprintf('%.3f',$row[$k]*100); $i++; } ?>];
        /*<?php $shiijan = array();  foreach($yearlist as $y){ $shijian[] = $y['year']; } ?>
        var shijian = eval(<?php echo json_encode($shijian); ?>);
        var shuju = [<?php $i=0; foreach($yearlist as $y){$year = $y['year'];$sql = "select * from xl_hs_detail where code='$code' and year='$year'";$row = $db->selectOne($sql);  echo $i=='0'?sprintf('%.3f',($row[$k]*100)):','.sprintf('%.3f',$row[$k]*100); $i++; } ?>];
        */
        option = {
            tooltip : {
                trigger: 'axis'
