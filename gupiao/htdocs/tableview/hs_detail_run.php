...
if($codelist){
  ...
  //      echo "<<<<<<<<==========================开始============================>>>>>>>>" . '\r';
        ...
        $year = date('Y');
        $sql = "select * from xl_hs_detail where code='$code' and roe !='0' and year < '$year' order by `year` desc";
        $list = $db->select($sql);
        $ygi = 0;
        $yglength = 10;
        $ygchange = FALSE;
        ...
        if ($ygchange) {
            $ygr15['yg_r15'] = 1;
            $db->update("xl_hs_huizong",$ygr15," code='$code'");
        } else {
            $sql = "update xl_hs_huizong set yg_r15='0' where `code`='$code'";
            $db->query($sql);
        }
