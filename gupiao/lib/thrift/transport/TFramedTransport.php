...
class TFramedTransport extends TTransport {
  ...
  public function read($len) {
    ...
    if ($len >= (strlen($this->rBuf_)-$this->readOffset_)) {
      $out = substr($this->rBuf_, $this->readOffset_);
      $this->rBuf_ = null;
      return $out;
    }

    // Return substr
    $out = substr($this->rBuf_, $this->readOffset_, $len);
    $this->readOffset_ += $len;

    if (strlen($this->rBuf_) <= $this->readOffset_) {
      $this->rBuf_ = null;
    }
    return $out;
  }
