...
class getInfo(object):
    ...
    def get(self, stock_code, stock_name):
        ...
        url = 'http://finance.sina.com.cn/realstock/company/sz' + str(stock_code) + '/nc.shtml'
        driver.get(url)
        hasInfo = True
        try:
            #实时股价
        ...
        if hasInfo:
            sql = "update xl_sz_real_time_share_price set stock_name='" + str(stock_name) + "',"\
...
"P_E_ratio='" + str(P_E_ratio) + "' where stock_code='" + str(stock_code) + "'"
            ...
            #print 'sql:' + sql
            if total_market_value_double > 0 and float(stock_price):
                #print 'sql:' + sql
...
if __name__ == '__main__':
    ...
    for stock_info in stock_code_infos:
            ...
            #    sql = "insert into xl_sz_real_time_share_price(stock_code) VALUES ('" + str(stock_code) + "')"
            #    cur_new.execute(sql)
            #    conn_new.commit()
            #except Exception,e:
