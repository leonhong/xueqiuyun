...
def get_real_time_info(stock_code, stock_name, market):
    ...
    if len(stock_price) > 0 and float(stock_price) > 0 and circulation_market_value_double > 0:
        ...
        sql="select * from xl_sh_real_time_share_price where stock_code='" + str(stock_code) + "'"
        rows=cur.fetchmany(cur.execute(sql))
        if len(rows)==0:
            sql="insert into xl_sh_real_time_share_price (stock_code, " \
                "stock_name, stock_price, total_market_value, stock_time) values(" \
                "'"+str(stock_code)+"','"+str(stock_name)+"','"+str(stock_price)+\
                "','"+str(circulation_market_value_double)+"','"+stock_time+"')"
        else:
...
if __name__ == '__main__':
    ...
    for stock_info in stock_code_infos:
        ...
        black_code=cur_new.execute("select * from code_black_list where code='" +stock_code+"'")
        balck_rows=cur_new.fetchmany(black_code)
        print stock_code
        print stock_name
        if len(balck_rows)==0:
            retryTimes = 0
